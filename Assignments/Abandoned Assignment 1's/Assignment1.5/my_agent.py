    ...
    def AgentFunction(self, percepts):
      ...
      """
        ...
        for x in range(self.word_length):
            if letter_states == 1:
                answer[x] = letter_indexes[x]

        for index in range(len(self.dictionary)):
            for x in range(self.word_length):
                if x in found_letters.keys():
                    if self.dictionary[index][x] == found_letters[x]:


        helper.letter_indices_to_word(answer, self.letters)
        ...
        print()
        print("Found letters: %s " % found_letters)
        print("Guess is: ")
