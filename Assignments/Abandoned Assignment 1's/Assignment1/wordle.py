...
class WordleGame:
    ...
    def __init__(self, language='english', word_length=5, verbose=False):
        ...
        self.solutions, self.dictionary, self.letters = read_dictionary(file_path=dictionary_file_path,
    ...
    def play(self, player, target, num_guesses, mode):
        ...
        while guess < num_guesses + 1:
            ...
            percepts = (guess, list(letter_indexes), list(letter_states))
            try:
                actions = player.agent.AgentFunction(percepts)
            except:
                print("Error! Failed to execute AgentFunction from '%s'" % player.playerFile)
                traceback.print_exc()
                sys.exit(-1)
            ...
            if not isinstance(actions, list) and not isinstance(actions, str):
                print(
                    "Error! AgentFunction from '%s.py' did not return a string nor list of indexes." % player.playerFile)
                sys.exit(-1)
                ...
                if np.sum(prev_greens) != np.sum(same_greens):
                    if self.verbose:
                        ...
                        if len(missing_greens) == 1:
                            print(f"%s does not have letter %s in the correct position!" % (
                                guessed_word, missing_greens_str))
                        else:
                            print(f"%s does not have letters %s in the correct positions!" % (
                                guessed_word, missing_greens_str))
                ...
                if len(missing_yellows) > 0:
                    if self.verbose:
                        ...
                        for i in range(len(missing_yellows)):
                            missing_yellows_str += f"{bcolors.INWORD}%c{bcolors.ENDC}" % self.letters[
                                missing_yellows[i]]
    ...
    def run(self, agentFile='agent_human.py', num_guesses=6, num_games=1000, seed=None, mode='easy', repeats=False):
        ...
        if repeats:
            I = rnd.randint(0, len(self.solutions), size=(num_games))
        else:
